import { createLotusRenderer } from "./scene/lotus-renderer.js?v=760320b28b75";
import { createDragonflyRenderer } from "./scene/dragonfly-renderer.js?v=760320b28b75";
import { initializePortfolio } from "./portfolio-controller.js?v=760320b28b75";
...
async function loadLotusRenderer() {
  try {
    const [lotusModel, vertexShader, fragmentShader] = await Promise.all([
      loadResource("../assets/models/lotus.json?v=760320b28b75", "json"),
      loadResource("../assets/shaders/lotus.vert?v=760320b28b75"),
      loadResource("../assets/shaders/lotus.frag?v=760320b28b75"),
    ]);
...
async function loadDragonflyRenderer(portfolioRoot) {
  try {
    const modelBuffer = await loadResource(
      "../assets/models/dragonfly.bin?v=760320b28b75",
      "arrayBuffer",
    );
...
async function startPortfolio() {
  ...
  const [portfolioData, lotusRenderer, dragonflyRenderer] = await Promise.all([
    loadResource("../data/projects.json?v=760320b28b75", "json"),
    loadLotusRenderer(),
